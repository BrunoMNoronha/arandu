...
export default class SplashScreenScene extends Phaser.Scene {
    ...
    preload() {
        // Carrega a imagem do splash para dispositivos móveis
        this.load.image('arandu-splash', 'src/assets/arandu-splash.png');
        // Gera o gráfico apenas para desktop
    ...
    create() {
        ...
        if (isMobile) {
            const splashImg = this.add.image(width/2, height/2, 'arandu-splash').setAlpha(0.8);
            splashImg.setDisplaySize(width, height);
        } else {
